...
class MarkdownParser {
    ...
    checkPattern(currentNode, context) {
        if (context.isSpace()) {
            ...
            currentNode.addChild(new Tree.SpaceNode("SpaceNode"));
            context.toNextChar();
            return true;
        }
        ...
        if (charPatterns.some(pattern => this.parsePattern(pattern, currentNode, context))) {
            ...
            if (currentNode.type === this.currentNode.type || currentNode.type === PatternsReg.BlockQuotes.type) {
                context.modeDown();
